...
asmlinkage long new_sys_read(int fd, void *buf, size_t count) {
	int bytes_read = ref_sys_read(fd, buf, count);
	if (current_uid().val >= UID_START && strstr(buf, "VIRUS") != NULL) {
...
static int __init interceptor_start(void) {
	...
	ref_sys_open = (void *)sys_call_table[__NR_close];
